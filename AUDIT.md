...
> **Read this first — provenance and status.**
>
> This audit was written on the `design-tokens-and-error-states` branch, which
> was never merged. Its Rev 2–7 fixes were made **there**, so a ✅ in the
> revision log below does **not** mean the finding is fixed in this tree.
>
> What actually carried across, and what did not, is recorded in
> **"Status in this branch"** immediately after the summary tables. That
> section is the one to trust; the revision log is kept because the reasoning
> in it is still worth reading.
>
> Checked against this tree on 5 September 2026.
...
Re-checked against `copy-and-palette-pass` on 5 September 2026. The ✅ marks in
the revision log refer to the other branch; these are the facts here.

| Finding | Here | Detail |
|---|---|---|
| **F1** landing bypasses the design system | **Partly fixed** | Hex down 45 → 13. The navy and every blue now come from `--onpoint-blue` / `--primary`. What is left is six subject-tile colours, four card pastels, gold, and two page backgrounds. Landing still forces light mode (`#F5F8FF` root, `bg-white/85` header) and is the one screen that ignores the dark theme by design. |
| **F2** error states missing | **Fixed** | `QueryError` ported; 14 of 23 query-bearing screens now tell a failed load from an empty one. Still without: `certificate`, `dream-world`, `penalty-shootout`, `treasure-island`, `visit`, `create-assignment`, `export`, `mark-submission`. |
| **F3** silent mutation errors | **STILL LIVE** | 11 mutations across `students.tsx` (5), `lessons.tsx` (3) and `resources.tsx` (3) have no `onError` between them. They handle `data.success === false`, but a thrown error passes silently — a delete over a flaky connection gives no feedback at all. |
| **F4** empty states that say the wrong thing | **STILL LIVE on two** | `lessons.tsx:899` "No lessons yet" and `resources.tsx:707` "Add your first learning resource" are both reached when filters exclude everything. The Grade Book now does this correctly and is the pattern to copy. |
| **F5** native browser dialogs | **STILL LIVE** | `window.confirm` at `dashboard.tsx:170`, `students.tsx:525`, `create-assignment.tsx:549`, in a codebase that uses `AlertDialog` properly in `submit-assignment.tsx`. |
| **F6** dead code | **Not done here** | The 26 unused symbols were removed on the other branch only. |
| **F7** hardcoded `hsl()` chart colours | **Not done here** | `reports.tsx` chart colours still sit outside the token system. |
| **F8** bimodal density | **STILL LIVE** | The student dashboard's marked-result row shows title and date but not the score, though `totalMarks` is already on the object. |

### Also true here, and not in the original audit

- The **four retired student pages are still in the tree** and were included in
  the platform copy pass before anyone noticed they were unreachable. Roughly
  777 lines of dead UI, plus `DreamBuilding.tsx` and `TownPlot.tsx`, which only
  those pages use.
- `text-[10px]` / `text-[11px]` and `rounded-[2rem]` now have real homes:
  `text-caption-01` and `rounded-lg` exist as of this branch.
- The legacy spacing burn-down list in `tailwind.config.ts` was recomputed
  against this tree: **342 uses across 13 keys**, not the 328 across 56 quoted
  further down.
