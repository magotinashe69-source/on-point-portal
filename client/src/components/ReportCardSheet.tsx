...
import {
  REPORT_TEXT, hasMarkedWork, versusClass, type ReportCard,
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", borderBottom: `2px solid ${NAVY}`, paddingBottom: "0.75rem" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ color: GOLD, fontStyle: "italic", fontSize: "0.75rem" }}>{REPORT_TEXT.tagline}</div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(11rem, 1fr))", gap: "0.35rem 1rem", margin: "0.9rem 0 1.1rem" }}>
        <Row label={REPORT_TEXT.student} value={card.student.fullName} />
        <Row label={REPORT_TEXT.pupilId} value={card.student.pupilId} />
        <Row label={REPORT_TEXT.form} value={card.student.form} />
        <Row label={REPORT_TEXT.term} value={card.term.label} />
        <Row label={REPORT_TEXT.issued} value={certificateDate(card.issuedAt)} />
      </div>

      {!anyWork ? (
        <p style={{ color: "#555", fontSize: "0.95rem", padding: "1.5rem 0" }} data-testid="text-nothing-marked">
          {REPORT_TEXT.nothingMarked}
        </p>
      ) : (
        <>
          {/* The subjects */}
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
              <thead>
                <tr>
                  {[REPORT_TEXT.subject, REPORT_TEXT.average, REPORT_TEXT.grade, REPORT_TEXT.classAverage, REPORT_TEXT.marked].map((h, i) => (
                    <th key={h} className={h === REPORT_TEXT.marked ? "rc-col-marked" : undefined} style={{
      ...
      <div style={{ marginTop: "1.1rem" }}>
        ...
        <div style={{ border: "1px solid #e5e7eb", minHeight: "4.5rem", padding: "0.6rem 0.7rem", marginTop: "0.3rem", fontSize: "0.88rem", color: "#222", whiteSpace: "pre-wrap" }}
          ...
          {card.comment || <span style={{ color: "#999" }}>{REPORT_TEXT.noComment}</span>}
        </div>
      </div>
