 ...
 *   describeError(err, "the Grade Book") -> "Could not load the Grade Book. ..."
 *
...
export function describeError(error: unknown, what = "this"): string {
  switch (statusOf(error)) {
    case "401":
      return "Your login has expired. Log in again to carry on.";
    case "403":
  ...
  return (
    <div className={`flex flex-col items-center justify-center ${pad} px-4 text-center`} data-testid={testId}>
      ...
      <div className="flex flex-wrap gap-2 mt-4 justify-center">
        {expired ? (
          <Link href={loginHref}>
            <Button size="sm" data-testid={`${testId}-login`}>Log in</Button>
          </Link>
        ) : (
          onRetry && (
