...
// The colours are fixed; the labels are read in whatever language the teacher
// is using, so this is built inside the component rather than at module load.
function chartConfigFor(t: Translation): ChartConfig {
  return {
    averagePercentage: {
      label: t.reports.averagePercent,
      color: "hsl(224, 65%, 35%)",
    },
    averageScore: {
...
export default function TeacherReports() {
  ...
  const getPerformanceBadge = (pct: number) => {
    if (pct >= 80) return <Badge className="bg-green-600">{t.reports.bandTop}</Badge>;
    if (pct >= 60) return <Badge>{t.reports.bandGood}</Badge>;
    if (pct >= 40) return <Badge variant="secondary">{t.reports.bandAverage}</Badge>;
    return <Badge variant="destructive">{t.reports.bandNeedsHelp}</Badge>;
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {isLoading ? (
          ...
          <QueryError error={error} what={t.errors.thing.theReport} variant="page" onRetry={() => refetch()} data-testid="reports-load-error" />
        ) : reportData ? (
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              <Select value={formFilter} onValueChange={setFormFilter}>
                <SelectTrigger className="w-40" data-testid="select-form-filter">
                  <SelectValue placeholder={t.reports.filterByForm} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t.reports.allForms}</SelectItem>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formFilter === "all" ? t.reports.allFormsNote : formFilter}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">{t.reports.classAverage}</p>
                </CardContent>
              </Card>

              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 gap-2">
                  <CardTitle className="text-sm font-medium">{t.reports.subjects}</CardTitle>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <Card>
                <CardHeader>
                  <CardTitle>{t.reports.studentPerformance}</CardTitle>
                  <CardDescription>{t.reports.studentPerformanceNote}</CardDescription>
                </CardHeader>
                <CardContent>
                  {filteredStudents.length > 0 ? (
                    ...
                    <p className="text-muted-foreground text-center py-8">{t.reports.noStudentData}</p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>{t.reports.subjectPerformance}</CardTitle>
                  <CardDescription>{t.reports.subjectPerformanceNote}</CardDescription>
                </CardHeader>
                <CardContent>
                  {reportData.subjectPerformance && reportData.subjectPerformance.length > 0 ? (
                    ...
                    <p className="text-muted-foreground text-center py-8">{t.reports.noSubjectData}</p>
                  )}
                </CardContent>
              </Card>
            </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t.reports.formComparison}</CardTitle>
                <CardDescription>{t.reports.formComparisonNote}</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    {reportData.formPerformance.map((form, index) => (
                      <Card key={form.form} className="border-2">
                        ...
                        <CardContent>
                          ...
                          <p className="text-sm text-muted-foreground">{t.reports.averageScoreLower}</p>
                        </CardContent>
                      </Card>
                    ))}
                  </div>
                ) : (
                  <p className="text-muted-foreground text-center py-8">{t.reports.noFormData}</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t.reports.studentDetails}</CardTitle>
                <CardDescription>{t.reports.studentDetailsNote}</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <table className="w-full text-sm" data-testid="table-student-details">
                    <thead>
                      <tr className="border-b">
                        <th className="text-left py-2 px-2">{t.reports.student}</th>
                        <th className="text-left py-2 px-2">Form</th>
                        <th className="text-center py-2 px-2">{t.reports.handedIn}</th>
                        <th className="text-center py-2 px-2">{t.reports.marked}</th>
                        <th className="text-center py-2 px-2">{t.reports.score}</th>
                        <th className="text-center py-2 px-2">Average</th>
                        <th className="text-center py-2 px-2">{t.reports.status}</th>
                      </tr>
                    </thead>
                    <tbody>
          ...
          <Card>
            <CardContent className="py-8 text-center">
              <p className="text-muted-foreground">{t.reports.noReportData}</p>
            </CardContent>
          </Card>
        )}
      </main>
